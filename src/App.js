...
class App extends Component {
  ...
  submitHandler = () => {
    ...
    let hasil= 0
    if(rumus==="persegi"){//parseINT gunanya unutk mengubah semua nilai menjadi int
    ...
    }//mengubah nilai hasi dari atas nya 
    this.setState({
      hasil,
  ...
  }                         //parameter adalah yang didalam kurung 
  changeScreenHandler = (name, val) => {//name diambil dari state yang diatas name adalah rumus val adalah persegi
    this.setState({
      [name]:val,
      submit:false
    })
  ...
  render() {
    return (
      <div>
        <center>
      {/* =>/arrow function berfungsi ketika fungsi memakai parameter jika tidak tidak perlu */}
          ...
          {!this.state.submit ? (
            ...
            {this.state.rumus === "persegi" &&
              ...
              </div>///fungsi biasa di render langsung jalan / => berguna untuk nahan melakukan event baru jalan berlaku pada fungsi
            }
